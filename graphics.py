'''GRAPHICS FILE ORIGINATES FROM SWINBURNE UNIVERSITY OF TECHNOLOGY, ARTIFICIAL INTELLIGENCE FOR GAMES'''
import pyglet, math
...
class GameWindow(pyglet.window.Window):
	...
	def add_handlers(self):
		...
		def on_draw():
			...
			self.batches["world"].draw()
			self.batches["car"].draw()
			if self.cfg["GUI"]:
				self.batches["gui"].draw()
...
settings = {
		...
		'width': int(get_param()[0]),
		'height': int(get_param()[1]),
		'vsync': True,
		'resizable': False,
		'caption': "Industry Visit Reflection Game",
	}
